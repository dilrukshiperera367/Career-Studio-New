"""
ClamAV antivirus integration for HRM file scanning.
...
def scan_file(file_obj: BinaryIO, filename: str = "upload") -> ScanResult:
    ...
    Returns ScanResult. Fails-open (is_clean=True) on connection errors
    so uploads are not blocked when ClamAV is temporarily unavailable.
    Enable strict mode with CLAMAV_ENABLED=True.
    """
